...
//                        producerService.send("kafka.tut", JSON.toJSONString(upLoadGasDto), requestData.getTerminalPort());
    ...
    @KafkaListener(groupId = "gas_kafka", id = "gas_kafka0", topicPartitions = { @TopicPartition(topic = TOPIC, partitions = { "0" }) })
    public void listen0(List<ConsumerRecord<?, ?>> records) {
        process(records);
    }

    @KafkaListener(groupId = "gas_kafka", id = "gas_kafka1", topicPartitions = { @TopicPartition(topic = TOPIC, partitions = { "1" }) })
    public void listen1(List<ConsumerRecord<?, ?>> records) {
        process(records);
    }

    @KafkaListener(groupId = "gas_kafka", id = "gas_kafka2", topicPartitions = { @TopicPartition(topic = TOPIC, partitions = { "2" }) })
