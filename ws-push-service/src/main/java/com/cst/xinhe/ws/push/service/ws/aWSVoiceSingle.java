...
@Component
public class aWSVoiceSingle {

    private static final Logger log = LoggerFactory.getLogger(aWSVoiceSingle.class);

    //静态变量，用来记录当前在线连接数。应该把它设计成线程安全的。
    private static int onlineCount = 0;
    //concurrent包的线程安全Set，用来存放每个客户端对应的MyWebSocket对象。
    private static CopyOnWriteArraySet<aWSVoiceSingle> webSocketSet = new CopyOnWriteArraySet<>();
...
//        }
    ...
    public static void sendInfo(String message) throws IOException {
        log.info(message);
        for (aWSVoiceSingle item : webSocketSet) {
            try {
                item.sendMessage(message);
            } catch (IOException e) {
                continue;
